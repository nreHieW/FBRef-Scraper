...
class FBRef:
    ...
    def get_season_link(self, year, league):
        ...
        for tag in soup.find_all("th", {"data-stat": ["year", "year_id"]}):
            finder_found = np.any(
                [f in tag.find("a")["href"] for f in finder if tag.find("a")]
            )  # bool, if any finders are found in tag
            season_found = np.any(
                [tag.getText() == s for s in calendar_years]
            )  # bool, if 1- or 2-calendar years are found in tag
    ...
    def get_match_links(self, year, league):
        ...
        scores_links = [
            t.find(href=True)
            for t in soup.find_all("td", {"data-stat": "score"})
            if t.find(href=True)
        ]
        if len(scores_links) == 0:
            print(
                f"No match score elements with links found at {fixtures_url} for {league} {year}."
            )
            return None
    ...
    def scrape_stats(self, year, league, stat_category, normalize=False):
        ...
        if normalize:
            ...
            per90_toggles = soup.find_all(
                "button", {"id": re.compile("per_match_toggle")}
            )
        ...
        squad_stats = (
            pd.read_html(str(squad_stats_tag))[0]
            if squad_stats_tag is not None
            else None
        )
        opponent_stats = (
            pd.read_html(str(opponent_stats_tag))[0]
            if opponent_stats_tag is not None
            else None
        )
        player_stats = (
            pd.read_html(str(player_stats_tag))[0]
            if player_stats_tag is not None
            else None
        )
        ...
        player_stats = player_stats[
            player_stats[("Unnamed: 0_level_0", "Rk")] != "Rk"
        ].reset_index(drop=True)
        ...
        if squad_stats is not None:
            squad_stats["Team ID"] = [
                ...
                for tag in squad_stats_tag.find_all("th", {"data-stat": "team"})[1:]
                if tag and tag.find("a")
            ]
        if opponent_stats is not None:
            opponent_stats["Team ID"] = [
                tag.find("a")["href"].split("/")[3]
                for tag in opponent_stats_tag.find_all("th", {"data-stat": "team"})[1:]
                if tag and tag.find("a")
            ]
        ...
        if player_stats is not None:
            player_links = [
                ...
                for tag in player_stats_tag.find_all("td", {"data-stat": "player"})
                if tag and tag.find("a")
            ]
    ...
    def scrape_match(self, link):
        ...
        # Matchweek/stage ==============================================================================================
        stage_el = list(
            soup.find("a", {"href": re.compile("-Stats")}, string=True).parents
        )[0]
        ...
        # Scores =======================================================================================================
        scores = soup.find("div", {"class": "scorebox"}).find_all(
            "div", {"class": "score"}
        )

        # Formations ===================================================================================================
        lineup_tags = [
            tag.find("table") for tag in soup.find_all("div", {"class": "lineup"})
        ]

        # Player stats =================================================================================================
        # Use table ID's to find the appropriate table. More flexible than xpath
        player_stats = dict()
        for i, (team, team_id) in enumerate(
            [("Home", home_team_id), ("Away", away_team_id)]
        ):
            summary_tag = soup.find_all(
                "table", {"id": re.compile(f"stats_{team_id}_summary")}
            )
            assert len(summary_tag) < 2
            summary_df = (
                pd.read_html(str(summary_tag[0]))[0] if len(summary_tag) == 1 else None
            )

            gk_tag = soup.find_all(
                "table", {"id": re.compile(f"keeper_stats_{team_id}")}
            )
            assert len(gk_tag) < 2
            gk_df = pd.read_html(str(gk_tag[0]))[0] if len(gk_tag) == 1 else None

            passing_tag = soup.find_all(
                "table", {"id": re.compile(f"stats_{team_id}_passing$")}
            )
            assert len(passing_tag) < 2
            passing_df = (
                pd.read_html(str(passing_tag[0]))[0] if len(passing_tag) == 1 else None
            )

            pass_types_tag = soup.find_all(
                "table", {"id": re.compile(f"stats_{team_id}_passing_types")}
            )
            assert len(pass_types_tag) < 2
            pass_types_df = (
                pd.read_html(str(pass_types_tag[0]))[0]
                if len(pass_types_tag) == 1
                else None
            )

            defense_tag = soup.find_all(
                "table", {"id": re.compile(f"stats_{team_id}_defense")}
            )
            assert len(defense_tag) < 2
            defense_df = (
                pd.read_html(str(defense_tag[0]))[0] if len(defense_tag) == 1 else None
            )

            possession_tag = soup.find_all(
                "table", {"id": re.compile(f"stats_{team_id}_possession")}
            )
            assert len(possession_tag) < 2
            possession_df = (
                pd.read_html(str(possession_tag[0]))[0]
                if len(possession_tag) == 1
                else None
            )

            misc_tag = soup.find_all(
                "table", {"id": re.compile(f"stats_{team_id}_misc")}
            )
            assert len(misc_tag) < 2
            misc_df = pd.read_html(str(misc_tag[0]))[0] if len(misc_tag) == 1 else None

            lineup_df = (
                pd.read_html(str(lineup_tags[i]))[0] if len(lineup_tags) != 0 else None
            )
            ...
            if summary_df is not None:
                ...
                for tag in summary_tag[0].find_all(
                    "th", {"data-stat": "player", "scope": "row", "class": "left"}
                ):
                    if tag.find("a"):
        ...
        # Expected stats flag ==========================================================================================
        expected = "Expected" in player_stats["Home"][
            "Summary"
        ...
        match["Home Formation"] = (
            player_stats["Home"]["Team Sheet"]
            .columns[0]
            .split("(")[-1]
            .replace(")", "")
            .strip()
            if player_stats["Home"]["Team Sheet"] is not None
            else None
        )
        match["Away Formation"] = (
            player_stats["Away"]["Team Sheet"]
            .columns[0]
            .split("(")[-1]
            .replace(")", "")
            .strip()
            if player_stats["Away"]["Team Sheet"] is not None
            else None
        )
        match["Home Goals"] = (
            int(scores[0].getText()) if scores[0].getText().isdecimal() else None
        )
        match["Away Goals"] = (
            int(scores[1].getText()) if scores[1].getText().isdecimal() else None
        )
        match["Home Ast"] = player_stats["Home"]["Summary"][
            ("Performance", "Ast")
        ].values[-1]
        match["Away Ast"] = player_stats["Away"]["Summary"][
            ("Performance", "Ast")
        ].values[-1]
        match["Home xG"] = (
        ...
        match["Home xAG"] = (
            player_stats["Home"]["Summary"][("Expected", "xAG")].values[-1]
            if expected
            else None
        )
        match["Away xAG"] = (
            player_stats["Away"]["Summary"][("Expected", "xAG")].values[-1]
            if expected
            else None
        )
    ...
    def scrape_complete_scouting_reports(self, year, league, goalkeepers=False):
        ...
        if goalkeepers:
            player_links = self.scrape_stats(year, league, "goalkeeping")[2][
                "Player Link"
            ].values
        else:
            player_links = self.scrape_stats(year, league, "standard")[2][
                "Player Link"
            ].values
        ...
        for player_link in tqdm(player_links):
            ...
            if (type(report) is int) or (
                not goalkeepers and per90["Position"].values[0] == "Goalkeepers"
            ):
                continue
    ...
    def complete_report_from_player_link(self, player_link):
        ...
        complete_report = pd.read_html(
            str(soup.find("table", {"id": re.compile(f"scout_full")}))
        )[
            0
        ]  # load report
        complete_report.columns = complete_report.columns.get_level_values(
            1
        )  # drop top level column name
        complete_report.dropna(axis=0, inplace=True)  # drop nan rows
        complete_report.reset_index(inplace=True, drop=True)  # reset index

        # Row masks
        header_row_mask = complete_report.eq(complete_report.iloc[:, 0], axis=0).all(
            1
        )  # rows with stats category header names

        #### Create multiindex column names broken down by stats category ####
        cleaned_complete_report = pd.DataFrame()
        stats_categories = ("Standard",) + tuple(
            complete_report[header_row_mask]["Statistic"].values
        )
        category_starts = [
            0,
        ] + list(
            np.where(header_row_mask)[0] + 2
        )  # 2 to skip past category name row and col name row
        category_ends = list(np.where(header_row_mask)[0] - 1) + [
            complete_report.shape[0] - 1
        ]
        ...
        cleaned_complete_report["Per 90"] = (
            cleaned_complete_report["Per 90"].str.rstrip("%").astype("float")
        )
        cleaned_complete_report["Percentile"] = cleaned_complete_report[
            "Percentile"
        ].astype(int)
